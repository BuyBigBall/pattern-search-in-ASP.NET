...
namespace BInarySearch
{
    ...
    class Program
    {
        ...
        {
            int len = path.Length;
            if (path.Substring(len - 4).ToLower() == ".csv" && (path.Length<10 || path.Substring(len - 10).ToLower() == "result.csv"))
            {
        ...
        public static void ProcessBinaryFile(string path)
        {
            ...
            if (path.Substring(len - 4).ToLower() == ".bin")
            {
                ...
                foreach (var a_pattern in pattern)
                {
                    ...
                    //        result.data = a_pattern.Value.data;
                    ...
                    //    }
                    //}
                    List<int> iIndexArray = searchObj.SearchPattern(a_pattern.Value.data, baBinaryData, a_pattern.Value.byteLen);
